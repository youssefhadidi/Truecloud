...
export function runClaude(opts) {
  return new Promise((resolve, reject) => {
    ...
    const args = [
      ...
      // Read is the only tool Claude needs to answer questions about a file
      // whose path we provide in the prompt. Glob/Grep aren't useful here and
      // their schemas would just inflate the input token count each turn.
      // Write/Edit/Bash are explicitly blocked in case the model ever tries.
      '--allowedTools', 'Read',
      '--disallowedTools', 'Write',
      '--disallowedTools', 'Edit',
      '--disallowedTools', 'Bash',
    ];
    // Override Claude Code's verbose coding-focused default system prompt
    // with a tight one tailored to file Q&A. Biggest single token saving.
