...
export function runClaude(opts) {
  return new Promise((resolve, reject) => {
    ...
    const args = [
      ...
      // Pre-approve only read-only tools. We deliberately do NOT pass
      // --permission-mode bypassPermissions because Claude Code refuses to
      // run that flag as root (security guard against destructive tool calls
      // running with full privileges). With these tools allowlisted, Claude
      // can answer questions about the file without prompting; any other
      // tool call (Write/Edit/Bash) is silently denied.
      '--allowedTools', 'Read',
      '--allowedTools', 'Glob',
      '--allowedTools', 'Grep',
