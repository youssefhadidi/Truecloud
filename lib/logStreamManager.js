...
import { readFile as readFileSync, writeFile as writeFileSync, appendFile } from 'fs/promises';
...
async function getLogState() {
  try {
    if (existsSync(STATE_FILE)) {
      const content = await readFileSync(STATE_FILE, 'utf-8');
...
async function saveLogState(path, offset) {
  try {
    await writeFileSync(STATE_FILE, JSON.stringify({ lastOffset: offset, lastPath: path }, null, 2));
  } catch (error) {
...
async function getLogHistory() {
  try {
    if (existsSync(HISTORY_FILE)) {
      const content = await readFileSync(HISTORY_FILE, 'utf-8');
      return JSON.parse(content);
    }
  } catch {
...
async function appendToHistory(newLines) {
  try {
    ...
    await writeFileSync(HISTORY_FILE, JSON.stringify(history, null, 2));
  } catch (error) {
...
async function checkAndBroadcastNewLogs() {
  try {
    ...
    for (const path of LOG_PATHS) {
      try {
        logContent = await readFileSync(path, 'utf-8');
        logPath = path;
        break;
      } catch {
        continue;
