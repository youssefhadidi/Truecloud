...
// Present while the cache worker's ffmpeg is writing into an HLS directory;
// holds that ffmpeg's { pid }. The two run in different processes and share
// nothing else, so this is how the on-demand job finds — and stops — a worker
// encode in the directory it is about to take over. Removing it is the signal:
// a worker whose lock is gone when its ffmpeg exits was preempted.
export const WORKER_LOCK = 'worker.lock';

// Bump whenever buildHlsArgs changes what ends up in a segment (codec, audio
// layout, timestamps). It is part of hlsRenditionHash, so every rendition made
// under the old args — finished or pre-cached — is re-made in a fresh
// directory, and the old ones age out through the pruner.
export const HLS_ARGS_VERSION = 2;

/**
 * Directory name (under stream-cache/hls/) for one rendition of one version of
 * a file. Shared by the on-demand job and the cache worker so they fill the
 * same directory.
 *
 * Everything that decides what ends up in a segment is part of the key: the
 * file version (size, mtime), the variant, the admin's maxHeight and
 * HLS_ARGS_VERSION. A finished rendition is only ever read back as "the
 * manifest says #EXT-X-ENDLIST", so a rendition made under other settings used
 * to keep being served, with nothing short of clearing the cache to replace
 * it. The key also names the segment URLs the browser caches, so they change
 * whenever the segments do.
 *
 * @param {string} fullPath Absolute path to the source
 * @param {{ size: number, mtimeMs: number }} st The source's stat
 * @param {{ variant?: 'h264'|'hevc', maxHeight?: number|null }} [options]
 */
export function hlsRenditionHash(fullPath, st, { variant = 'h264', maxHeight = null } = {}) {
  return createHash('md5')
    .update(`${fullPath}|${st.size}|${st.mtimeMs}|${variant}|${maxHeight ?? 'source'}|v${HLS_ARGS_VERSION}`)
    .digest('hex');
}
