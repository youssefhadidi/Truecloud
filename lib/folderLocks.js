...
export async function requireFolderUnlock(req, relativePath) {
  ...
  let pin = req.headers.get('x-folder-pin');
  if (!pin) {
    try {
  ...
  if (!pin) {
    // X-Folder-Pins carries the full {lockPath: pin} map from the axios
    // interceptor — used when the request's target path lives in the body
    // (e.g. /api/files/move) so the interceptor can't pick a single PIN.
    const raw = req.headers.get('x-folder-pins');
    if (raw) {
      try {
        const map = JSON.parse(raw);
        if (map && typeof map === 'object' && typeof map[ancestor] === 'string') {
          pin = map[ancestor];
        }
      } catch {
        pin = null;
