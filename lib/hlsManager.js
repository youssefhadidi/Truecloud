 ...
 *
 * Output directory: stream-cache/hls/{md5(fullPath, size, mtime)}/
...
// Keyed on size and mtime as well as the path: a file replaced in place must
// get a fresh rendition, not the old file's segments (whose presence alone
// reads as a finished transcode). The old directory is left to the pruner.
//
// The H.264 key is unchanged from before variants existed (and must match the
// cache worker's hlsHash); other variants append their name, so each gets its
// own directory and its own entry in `jobs`.
export function getHlsHash(fullPath, st, variant = 'h264') {
  const suffix = variant === 'h264' ? '' : `|${variant}`;
  return createHash('md5').update(`${fullPath}|${st.size}|${st.mtimeMs}${suffix}`).digest('hex');
}

/** Rejects if the source file is missing. */
export async function getHlsOutputDir(fullPath, cacheDir, variant = 'h264') {
  const hash = getHlsHash(fullPath, await stat(fullPath), variant);
 ...
 */
export async function isHlsCacheComplete(fullPath, cacheDir, variant = 'h264') {
  try {
    const { hlsDir } = await getHlsOutputDir(fullPath, cacheDir, variant);
...
export async function startHlsJob(fullPath, cacheDir, codecs, hwaccel, durationSecs, { maxHeight, variant = 'h264' } = {}) {
  ...
  const { hash, hlsDir } = await getHlsOutputDir(fullPath, cacheDir, variant);

  // Already complete on disk
  const complete = await isHlsCacheComplete(fullPath, cacheDir, variant);
  if (complete) {
    jobs.delete(hash);
