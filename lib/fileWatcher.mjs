...
import { IGNORED_DIR_NAMES, isCachePath } from './cachePaths.mjs';
...
// Matched with String.includes, so the undotted 'stream-cache' also covers a
// '.stream-cache' directory — the two spellings both exist in the wild because
// the default used to differ between modules.
const IGNORED_PATTERNS = IGNORED_DIR_NAMES;
...
export function startFileWatcher() {
  ...
  watcher = chokidar.watch(UPLOAD_DIR, {
    // Cache dirs configured inside UPLOAD_DIR are matched by resolved path, not
    // by name — a folder literally called 'cache' is not otherwise ignored.
    ignored: (path) => IGNORED_PATTERNS.some((pattern) => path.includes(pattern)) || isCachePath(path),
    persistent: true,
    awaitWriteFinish: {
  ...
  watcher
    ...
    .on('error', (error) => {
      if (error?.code === 'EINVAL' || error?.code === 'ENOSPC' || error?.code === 'EACCES' || error?.code === 'EPERM') {
        logger.warn('FileWatcher: ignored fs error', { code: error.code, path: error.path });
        return;
      }
