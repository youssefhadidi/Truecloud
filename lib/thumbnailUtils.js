...
// Shared across the pages-router upload handlers and the app-router thumbnail
// routes, which Next bundles separately — hence globalThis rather than module
// scope, or each bundle would get its own limiter and in-flight map.
const jobs = (globalThis.__thumbnailJobs ??= {
  // Overall cap on concurrent thumbnail generations (sharp / ffmpeg / magick)
  semaphore: new Semaphore(20),
  // Upload-triggered jobs may hold at most this many of the slots above, so a
  // bulk upload's backlog never queues ahead of thumbnails a user is viewing.
  backgroundSemaphore: new Semaphore(2),
  // thumbnailPath -> Promise, so concurrent requests for one thumbnail share a
  // single generation instead of racing to write the same file.
  inFlight: new Map(),
});

/**
 * Run `generate` to produce `thumbnailPath`, deduplicated against any job
 * already producing the same file and bounded by the shared concurrency limit.
 * @param {string} thumbnailPath
 * @param {() => Promise<void>} generate
 * @param {{ background?: boolean }} [options] - background jobs yield to
 *   on-demand ones by holding at most a couple of slots.
 */
export function runThumbnailJob(thumbnailPath, generate, { background = false } = {}) {
  const existing = jobs.inFlight.get(thumbnailPath);
  // Join the existing job, unless it's a background job still queued behind an
  // upload backlog — then an on-demand request takes over and the queued one
  // is skipped when its turn comes.
  if (existing) {
    if (background || existing.started || !existing.background) return existing.promise;
    existing.superseded = true;
  }

  const entry = { background, started: false, superseded: false, promise: null };
  entry.promise = (async () => {
    if (background) await jobs.backgroundSemaphore.acquire();
    try {
      if (entry.superseded) return;
      await jobs.semaphore.acquire();
      try {
        if (entry.superseded) return;
        entry.started = true;
        await generate();
      } finally {
        jobs.semaphore.release();
      }
    } finally {
      if (background) jobs.backgroundSemaphore.release();
    }
  })().finally(() => {
    if (jobs.inFlight.get(thumbnailPath) === entry) jobs.inFlight.delete(thumbnailPath);
  });
  jobs.inFlight.set(thumbnailPath, entry);
 ...
 * Writing in place let a request that arrived mid-generation (from the cache
 * worker, which the in-flight map can't see) or after a crash mid-write serve a
...
export async function generateImageThumbnail(filePathOrBuffer, thumbnailPath) {
  ...
  await writeAtomically(thumbnailPath, (tmpPath) =>
    sharp(filePathOrBuffer, { failOn: 'none', failOnError: false, limitInputPixels: false })
      .rotate()
...
export async function generateVideoThumbnail(filePath, thumbnailPath, timeoutMs = 20000) {
  return writeAtomically(thumbnailPath, (tmpPath) => extractVideoFrame(filePath, tmpPath, timeoutMs));
}

async function extractVideoFrame(filePath, thumbnailPath, timeoutMs) {
  const { size, quality } = await readThumbnailConfig();
...
export async function generatePdfThumbnail(filePath, thumbnailPath, timeoutMs = 60000) {
  return writeAtomically(thumbnailPath, async (tmpPath) => {
    const jpgPath = tmpPath.replace(/\.webp$/, '.jpg');
    try {
      await renderPdfThumbnail(filePath, jpgPath, tmpPath, timeoutMs);
    } finally {
      // Ghostscript leaves a partial page behind on failure or timeout.
      await fsPromises.unlink(jpgPath).catch(() => {});
    }
  });
}

async function renderPdfThumbnail(filePath, jpgPath, thumbnailPath, timeoutMs) {
  const { size, quality } = await readThumbnailConfig();

  // Render at ~2x the thumbnail's long edge for a typical page (A4 / Letter
  // are ~11-11.7in tall) instead of a fixed 150 DPI: that was ~2 MP per page
  // just to be shrunk to `size`, and made scanned / large-format PDFs slow
  // enough to hold connections the viewer itself needed. Anti-aliasing keeps
  // text legible at the low resolution.
  const dpi = Math.max(36, Math.ceil((size * 2) / 11));
  const gsArgs = [
    '-q', '-dNOPAUSE', '-dBATCH', '-dSAFER', '-sDEVICE=jpeg', '-dJPEGQ=90',
    '-dFirstPage=1', '-dLastPage=1', '-dUseCropBox', `-r${dpi}`,
    '-dTextAlphaBits=4', '-dGraphicsAlphaBits=4',
    `-sOutputFile=${jpgPath}`, filePath,
  ];
...
export function generateThumbnailForUpload(filePath, relativePath, fileName) {
  ...
  (async () => {
    try {
      ...
      await runThumbnailJob(
        thumbnailPath,
        () => {
          if (isImage) return generateImageThumbnail(filePath, thumbnailPath);
