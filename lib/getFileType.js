...
import { isImage, isVideo, isAudio, isPdf, isXlsx, is3dFile, isText } from '@/lib/clientFileUtils';
 ...
 * @returns {string|null} - File type: '3d', 'image', 'video', 'audio', 'pdf', 'xlsx', 'text', or null
 */
export function getFileType(file) {
  ...
  // Last: the media lists win any overlap (svg is an image, ts is video).
...
export function isViewableFile(file) {
  const fileName = typeof file === 'string' ? file : file?.name;
  return !!(fileName && (isImage(fileName) || isVideo(fileName) || isAudio(fileName) || is3dFile(fileName) || isPdf(fileName) || isXlsx(fileName) || isText(fileName)));
}
