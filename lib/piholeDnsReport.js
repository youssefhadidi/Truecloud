...
async function runSqlite(sql, params) {
    ...
    for (const flags of [['-readonly', '-json'], ['-json']]) {
      try {
        const { stdout } = await execFileAsync(
          cmd,
          [...prefix, ...flags, FTL_DB_PATH, sql, ...params.map(String)],
  ...
  throw new PiholeError(`Could not read the Pi-hole query database: ${detail}`, {
    status: 502,
    hint: `Truecloud reads ${FTL_DB_PATH} directly via "pihole-FTL sqlite3". Check that Pi-hole is installed on this host and that the server process can read that file.`,
  });
}
...
export async function getClientCategoryReport({ hours = 24, topDomains = 8 } = {}) {
  ...
  const rows = parseRows(await runSqlite(SQL, [since]));
