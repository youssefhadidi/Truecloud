...
export function addList(address, { type = 'block', comment = '', enabled = true, groups = [0] } = {}) {
  ...
  return piholeFetch('/api/lists', {
    method: 'POST',
    body: { address: url, type, comment, enabled: Boolean(enabled), groups },
  });
}
...
export async function runGravity(onLine) {
  ...
  const tail = cleanLine(buffer + decoder.decode());
  if (tail) onLine?.(tail);
}
...
// Gravity output is meant for a terminal: strip ANSI codes and carriage
// returns so the job log stays readable.
