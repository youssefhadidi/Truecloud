 ...
 * Invalidates the files list cache when download starts.
 */
export function useStartDownload() {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      // Invalidate files list for the download path (from API response)
 ...
 * Resolves with { filesDeleted } — false when a finished download was merely
 * dismissed, true when an unfinished one had its partial data deleted.
 */
...
/**
 * Hook to dismiss the whole completed history. Downloaded files are untouched;
 * the service broadcasts a download-removed per entry so open clients follow.
 */
export function useClearCompleted() {
  return useMutation({
    mutationFn: async () => {
      const response = await axios.patch('/api/files/torrent-download', { action: 'clear-completed' });
      return response.data;
    },
  });
}
