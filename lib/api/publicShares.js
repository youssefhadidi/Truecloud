...
/**
 * Refetch everything tied to a share's visitor identity. The email lives in an
 * HttpOnly cookie, so it isn't part of the query keys.
 */
function resetShareIdentityQueries(queryClient, token) {
  queryClient.removeQueries({ queryKey: ['share-files', token] });
  queryClient.invalidateQueries({ queryKey: ['share', token] });
}

/**
 * Hook to set the visitor email on a private-uploads share
 */
export function useIdentifyShare() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async ({ token, sharePassword, email }) => {
      const headers = sharePassword ? { 'x-share-password': sharePassword } : {};
      const response = await axios.post(`/api/public/${token}/identify`, { email }, { headers });
      return response.data;
    },
    onSuccess: (_, { token }) => resetShareIdentityQueries(queryClient, token),
  });
}

/**
 * Hook to forget the visitor email on a private-uploads share
 */
export function useClearShareIdentity() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async ({ token }) => {
      const response = await axios.delete(`/api/public/${token}/identify`);
      return response.data;
    },
    onSuccess: (_, { token }) => resetShareIdentityQueries(queryClient, token),
  });
  ...
  return useMutation({
    mutationFn: async ({ token, sharePassword, file, currentSubPath }) => {
      ...
      const response = await axios.post(
        ...
        formData,
        // Same ceiling as authenticated uploads: a dead mobile connection
        // should eventually fail rather than spin in the upload list forever
