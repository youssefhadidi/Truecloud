...
/**
 * Query options for a folder listing, shared by useFiles and the hover
 * prefetch so both fill the same cache entry.
 */
export function filesQueryOptions(currentPath) {
  return {
...
/**
 * Hook to upload a file. Doesn't refresh the listing itself: callers upload in
 * batches and refresh once per batch (the file-change WebSocket event also
 * refreshes it as each file lands).
 */
export function useUploadFile(onProgress) {
  return useMutation({
    mutationFn: async ({ file, uploadId, path }) => {
      const formData = new FormData();
      formData.append('file', file, file?.name);

      // Progress events fire far more often than the rounded % changes;
      // only report actual changes so the transfer panel isn't re-rendered
      // on every event.
      let lastProgress = -1;

      const response = await axios.post(`/api/files/upload?path=${encodeURIComponent(path)}`, formData, {
        headers: {
...
/**
 * Hook to fetch only folders from a specific path
 * Used for move/copy dialogs
 */
export function useFolders(path) {
  return useQuery({
...
/**
 * Identify one version of a file for cache-busting media URLs: its size and
 * mtime, as the listing reports them. A URL carrying it may be cached by the
 * browser indefinitely (see mediaCacheControl in lib/httpRange.js); '' when the
 * caller's file object doesn't have them, which makes the browser revalidate.
 */
export function fileVersion(file) {
  if (file?.size == null || !file?.updatedAt) return '';
  const mtime = new Date(file.updatedAt).getTime();
  return Number.isFinite(mtime) ? `${file.size}-${mtime}` : '';
}
 ...
 */
export function getThumbnailUrl(fileId, filePath = '', version = '') {
  if (!fileId) return null;
  const v = version ? `&v=${encodeURIComponent(version)}` : '';
  // <img src> can't carry the X-Folder-Pins header, so embed the PIN as a
  // query param when this thumbnail lives inside a passcode-locked folder.
  return appendFolderPinToUrl(
    `/api/files/thumbnail/${encodeURIComponent(fileId)}?path=${encodeURIComponent(filePath || '')}${v}`,
    filePath || '',
  );
}
 ...
 */
export function getShareThumbnailUrl(shareToken, fileName, filePath = '', sharePassword = '', version = '') {
...
// Module-level so React Query can memoize its result: an inline select is a new
// function each render, so it re-runs and hands the grid a new Set every time.
function selectSharedPaths(shares) {
...
/**
 * Hook to fetch uploaders of a private-uploads share (grouped by email)
 */
export function useShareUploads(shareId, enabled = true) {
  return useQuery({
    queryKey: ['shareUploads', shareId],
    queryFn: async () => {
      const response = await axios.get(`/api/shares/${shareId}/uploads`);
...
/**
 * Helper function to fetch folders (for use in queryClient.fetchQuery)
 * Returns only directories from a path
 */
export async function fetchFoldersHelper(path) {
  const response = await axios.get(`/api/files?path=${encodeURIComponent(path)}`);
  const allFiles = response.data.files || [];
  return allFiles.filter((file) => file.isDirectory);
}
