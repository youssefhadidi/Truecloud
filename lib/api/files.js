...
/**
 * Hook to fetch share thumbnail (generates if needed, returns base64)
 */
export function useShareThumbnail(shareToken, fileName, filePath, sharePassword, enabled = true) {
  return useQuery({
    queryKey: ['share-thumbnail', shareToken, fileName, filePath, sharePassword],
    queryFn: async () => {
      ...
      const response = await fetch(`/api/public/${shareToken}/thumbnail?${params.toString()}`);
      const data = await response.json();
      if (!response.ok) {
        throw new Error(data.error || 'Thumbnail not found');
      }
      return data; // { data: "data:image/webp;base64,...", generated: boolean }
    },
    retry: 1,
    staleTime: Infinity,
    cacheTime: Infinity,
