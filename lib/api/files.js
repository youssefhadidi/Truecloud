    ...
    mutationFn: async ({ file, uploadId }) => {
      return new Promise((resolve, reject) => {
        ...
        xhr.upload.addEventListener('progress', (e) => {
          console.log('useUploadFile - Upload progress:', {
            loaded: e.loaded,
            total: e.total,
            percent: e.lengthComputable ? Math.round((e.loaded / e.total) * 100) : 'unknown',
          });
        ...
        xhr.addEventListener('load', () => {
          console.log('useUploadFile - Upload load event:', {
            status: xhr.status,
        ...
        xhr.addEventListener('error', (e) => {
          console.error('useUploadFile - XHR error event:', {
            type: e.type,
            message: e.message,
          });
        ...
        xhr.addEventListener('abort', (e) => {
          console.error('useUploadFile - XHR abort event:', {
            type: e.type,
            message: e.message,
          });
        ...
        xhr.addEventListener('timeout', (e) => {
          console.error('useUploadFile - XHR timeout event:', {
            type: e.type,
            timeout: xhr.timeout,
          });
        ...
        // Set a very long timeout (30 minutes) for large file uploads
        xhr.timeout = 30 * 60 * 1000; // 30 minutes in milliseconds
        console.log('useUploadFile - XHR timeout set to:', xhr.timeout, 'ms');

        // Use native FormData - browser handles multipart encoding correctly
        const formData = new FormData();

        // Debug: Log file object
        console.log('useUploadFile - File object:', {
          name: file?.name,
          size: file?.size,
          type: file?.type,
        ...
        // Debug: Check what's in formData (FormData is not directly inspectable, but we can check its entries)
        if (typeof FormData !== 'undefined' && formData.entries) {
          try {
            console.log('useUploadFile - FormData entries:');
            ...
            console.error('Could not enumerate FormData:', e);
          }
        }
...
export function useThumbnail(fileId, filePath, enabled = true) {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await axios.get(`/api/files/thumbnail/${encodeURIComponent(fileId)}?path=${encodeURIComponent(filePath)}`);
