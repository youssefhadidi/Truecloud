...
/**
 * Hook to fetch the module library (available modules)
 */
export function useModuleLibrary() {
  const { data, isPending, ...rest } = useQuery({
    queryKey: ['module-library'],
    queryFn: async () => {
      const response = await axios.get('/api/admin/modules/library');
      return response.data.modules || [];
    },
  });
...
/**
 * Hook to add a module to the library
 */
export function useAddToLibrary() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async ({ name, description, repository }) => {
      const response = await axios.post('/api/admin/modules/library', { name, description, repository });
      return response.data;
    },
    onSuccess: () => {
...
/**
 * Hook to remove a module from the library
 */
export function useRemoveFromLibrary() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async (repository) => {
      const response = await axios.delete('/api/admin/modules/library', { data: { repository } });
      return response.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['module-library'] });
