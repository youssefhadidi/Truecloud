/** @format */

import { useQuery, useMutation } from '@tanstack/react-query';
import axios from '@/lib/axiosConfig';

/**
 * Hook to fetch system requirements
 */
export function useSystemRequirements() {
  const { data, isPending, ...rest } = useQuery({
    queryKey: ['systemRequirements'],
    queryFn: async () => {
      const response = await axios.get('/api/system/check-requirements');
      return response.data.requirements || [];
    },
  });

  const isLoading = isPending && !data;

  return {
    data,
    isPending,
    isLoading,
    ...rest,
  };
}

/**
 * Hook to install a system requirement
 */
export function useInstallRequirement() {
  return useMutation({
    mutationFn: async (name) => {
      const response = await axios.post('/api/system/install-requirement', { name });
      return response.data;
    },
  });
}

/**
 * Hook to check for updates
 */
export function useCheckUpdates(enabled = false) {
  const { data, isPending, ...rest } = useQuery({
    queryKey: ['checkUpdates'],
    queryFn: async () => {
      const response = await axios.get('/api/system/check-updates');
      return response.data;
    },
    enabled,
    staleTime: 5 * 60 * 1000, // 5 minutes
  });

  const isLoading = isPending && !data;

  return {
    data,
    isPending,
    isLoading,
    ...rest,
  };
}

/**
 * Hook to run update. Pass a target ('app' or 'torrent-service') to choose
 * which checkout gets updated; defaults to the main app.
 */
export function useRunUpdate() {
  return useMutation({
    mutationFn: async (target = 'app') => {
      const response = await axios.post('/api/system/run-update', { target });
      return response.data;
    },
  });
}

/**
 * Hook to fetch system update status
 */
export function useUpdateStatus() {
  const { data, isPending, ...rest } = useQuery({
    queryKey: ['updateStatus'],
    queryFn: async () => {
      const response = await axios.get('/api/system/update-status');
      return response.data;
    },
    // Don't automatically refetch since WebSocket handles real-time updates
    refetchOnWindowFocus: false,
    staleTime: Infinity,
  });

  const isLoading = isPending && !data;

  return {
    data,
    isPending,
    isLoading,
    ...rest,
  };
}

/**
 * Hook to fetch thumbnail settings
 */
export function useThumbnailSettings() {
  const { data, isPending, ...rest } = useQuery({
    queryKey: ['thumbnailSettings'],
    queryFn: async () => {
      const response = await axios.get('/api/admin/thumbnail-settings');
      return response.data;
    },
  });

  const isLoading = isPending && !data;

  return {
    data,
    isPending,
    isLoading,
    ...rest,
  };
}

/**
 * Hook to save/reset thumbnail settings
 */
export function useSaveThumbnailSettings() {
  return useMutation({
    mutationFn: async (settings) => {
      const response = await axios.put('/api/admin/thumbnail-settings', settings);
      return response.data;
    },
  });
}

/**
 * Hook to fetch components config
 */
export function useComponentsConfig(enabled = true) {
  const { data, isPending, ...rest } = useQuery({
    queryKey: ['componentsConfig'],
    queryFn: async () => {
      const response = await axios.get('/api/admin/components');
      return response.data;
    },
    enabled,
    staleTime: 30_000,
  });

  const isLoading = isPending && !data;

  return {
    data,
    isPending,
    isLoading,
    ...rest,
  };
}

/**
 * Hook to fetch transcoding settings
 */
export function useTranscodingSettings() {
  const { data, isPending, ...rest } = useQuery({
    queryKey: ['transcodingSettings'],
    queryFn: async () => {
      const response = await axios.get('/api/admin/transcoding-settings');
      return response.data;
    },
  });

  const isLoading = isPending && !data;

  return {
    data,
    isPending,
    isLoading,
    ...rest,
  };
}

/**
 * Hook to save transcoding settings
 */
export function useSaveTranscodingSettings() {
  return useMutation({
    mutationFn: async (settings) => {
      const response = await axios.put('/api/admin/transcoding-settings', settings);
      return response.data;
    },
  });
}

/**
 * Hook to save components config
 */
export function useSaveComponentsConfig() {
  return useMutation({
    mutationFn: async (config) => {
      const response = await axios.put('/api/admin/components', config);
      return response.data;
    },
  });
}
