 ...
 * Falls back to the fallback locale, then to the key itself, so a missing
...
export function translate(lang, key, vars) {
  if (key == null) return '';
  const active = dictionaries[lang] || dictionaries[FALLBACK_LOCALE];
  let value = lookup(active, key);
  if (value === undefined && lang !== FALLBACK_LOCALE) {
