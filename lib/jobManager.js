...
/**
 * Record the most recent log line for a job. Only the latest entry is kept —
 * full log history would balloon WS broadcasts (the job is re-sent on every
 * status/progress change), so callers should rely on the system logs viewer
 * for detailed inspection.
 */
export function addJobLog(id, message, level = 'info') {
  const job = global.jobs.get(id);
  if (!job) return;
  job.lastLog = { time: new Date().toISOString(), message: String(message), level };
  broadcastJob(id);
}
  ...
  if (status.isRunning) {
    ...
    if (status.currentFile) {
      ...
      if (job) {
        ...
        if (!job.lastLog || job.lastLog.message !== msg) {
