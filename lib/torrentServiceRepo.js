 ...
 * Honour whichever lockfile is committed, then fall back to pnpm (what
 * TORRENT.md documents) when it's actually installed.
 */
export function detectPackageManager(path) {
  ...
  try {
    execSync('command -v pnpm', { stdio: 'pipe', timeout: 5000 });
    return 'pnpm';
  } catch {
    return 'npm';
  }
