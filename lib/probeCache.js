 ...
 *
 * Entry shape:
 *   { size, mtimeMs, videoCodec, audioCodec, videoHeight, pixFmt, dvProfile, durationSecs }
 */
...
function isValidEntry(entry) {
  return (
    ...
    'videoCodec' in entry &&
    'audioCodec' in entry &&
    // Added later; entries from before it re-probe once rather than read as
    // "not Dolby Vision" for a file that might be.
    'dvProfile' in entry
  );
}
...
export async function getProbeInfo(fullPath, signal) {
  ...
  const work = (async () => {
    ...
    const entry = {
      ...
      dvProfile: codecs.dvProfile ?? null,
