 ...
 */
export function buildHlsArgs(inputPath, outputM3u8, videoCodec, audioCodec, hwaccel) {
  const needsAudioTranscode = !isAudioBrowserCompatible(audioCodec);
  ...
  // Video is already H.264: copy stream, only fix audio
  if (videoCodec === 'h264') {
    return ['-i', inputPath, '-c:v', 'copy', ...hlsTail];
  }

  // VAAPI path: software decode → NV12 → VAAPI upload → h264_vaapi encode.
  // Using -vaapi_device (not -hwaccel vaapi) so the decoder always runs in software,
  // which supports any input codec (HEVC, VP9, etc.) regardless of GPU decode capability.
  if (hwaccel === 'vaapi') {
    return [
  ...
  return ['-i', inputPath, '-c:v', 'libx264', '-crf', '23', '-preset', 'fast', ...hlsTail];
}
