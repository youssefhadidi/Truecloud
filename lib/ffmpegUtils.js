...
export function buildHlsArgs(inputPath, outputM3u8, videoCodec, audioCodec, hwaccel, { maxHeight } = {}) {
  ...
  if (hwaccel === 'vaapi') {
    // SW decode → NV12 → VAAPI upload → HW encode.
    // We previously kept frames on the GPU end-to-end (HW decode →
    // scale_vaapi → h264_vaapi) for codecs the iGPU could decode, but
    // scale_vaapi refuses to configure its output pad across several
    // drivers when fed 10-bit HEVC (Main 10 HMAX/Netflix rips). Doing
    // decode + scale in software and only uploading NV12 for encode is
    // universally compatible; the expensive stage (encode) still runs on
    // the GPU.
    const vaapiFilter = scaleFilter
      ? `${scaleFilter},format=nv12,hwupload`
      : 'format=nv12,hwupload';
