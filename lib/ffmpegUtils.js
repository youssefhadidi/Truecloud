...
export function buildHlsArgs(inputPath, outputM3u8, videoCodec, audioCodec, hwaccel, { maxHeight } = {}) {
  const needsAudioTranscode = !isAudioBrowserCompatible(audioCodec);
  const audioArgs = needsAudioTranscode ? ['-c:a', 'aac', '-b:a', '192k'] : ['-c:a', 'copy'];
  ...
  // Video is already H.264: copy if no rescale needed, else re-encode with libx264
  if (videoCodec === 'h264') {
    if (!scaleFilter) return ['-i', inputPath, '-c:v', 'copy', ...hlsTail];
    return ['-i', inputPath, '-c:v', 'libx264', '-crf', '23', '-preset', 'fast', '-vf', scaleFilter, ...hlsTail];
  }

  // VAAPI path: software decode → (scale) → NV12 → VAAPI upload → h264_vaapi encode.
  // Using -vaapi_device (not -hwaccel vaapi) so the decoder always runs in software,
  // which supports any input codec (HEVC, VP9, etc.) regardless of GPU decode capability.
  if (hwaccel === 'vaapi') {
    const vaapiFilter = scaleFilter
  ...
  const args = ['-i', inputPath, '-c:v', 'libx264', '-crf', '23', '-preset', 'fast'];
  if (scaleFilter) args.push('-vf', scaleFilter);
  return [...args, ...hlsTail];
}
