...
// Video codecs the UHD 630 / Gen9.5 iGPU can decode in hardware via VAAPI.
// AV1 is excluded (Tiger Lake+ only); exotic formats (VC-1, MPEG-4 ASP) are
// excluded for reliability — they fall through to software decode.
const HW_DECODE_CODECS = new Set(['h264', 'hevc', 'vp9', 'vp8', 'mpeg2video']);
 ...
 */
export function buildHlsArgs(inputPath, outputM3u8, videoCodec, audioCodec, hwaccel, { maxHeight } = {}) {
  const needsAudioTranscode = !isAudioBrowserCompatible(audioCodec);
  ...
  // Scale filter: caps resolution without upscaling. -2 keeps width a multiple of 2.
  ...
  if (videoCodec === 'h264' && !scaleFilter) {
    return ['-i', inputPath, '-c:v', 'copy', ...hlsTail];
  }

  // VAAPI encode args — shared by both the HW-decode and SW-decode branches.
  ...
  if (hwaccel === 'vaapi') {
    // HW decode path: frames stay in GPU memory end-to-end (decode → optional
    // scale_vaapi → h264_vaapi encode). Huge CPU win on HEVC/VP9 sources which
    // would otherwise burn multiple cores on software decode.
    // Gated on codec name because the iGPU can only decode known formats (no AV1
    // on Gen9.5 / UHD 630). If the actual stream uses an unsupported pixel format
    // (e.g. 10-bit 4:4:4), ffmpeg exits and hlsManager retries with software libx264.
    if (HW_DECODE_CODECS.has(videoCodec)) {
      const args = [
        '-hwaccel', 'vaapi',
      ...
      // format=nv12 downconverts 10-bit P010 (Main 10 HEVC, VP9 Profile 2) to
      // 8-bit NV12 — required because h264_vaapi encoders generally don't accept
      // P010. Without it, scale_vaapi fails to configure its output pad.
      const vaapiVf = scaleFilter
        ? `scale_vaapi=w=-2:h=${maxHeight}:format=nv12`
        : 'scale_vaapi=format=nv12';
      args.push('-vf', vaapiVf);
      return [...args, ...vaapiEncodeArgs, ...hlsTail];
    }

    // SW decode → NV12 → VAAPI upload → HW encode. Fallback for codecs the iGPU
    // can't decode in hardware (AV1, exotic formats).
    const vaapiFilter = scaleFilter
      ? `${scaleFilter},format=nv12,hwupload`
      : 'format=nv12,hwupload';
    return [
