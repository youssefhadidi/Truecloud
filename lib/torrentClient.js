...
async function apiFetch(path, { timeoutMs = FETCH_TIMEOUT_MS, ...options } = {}) {
...
// `timeoutMs` overrides the default for callers on a latency-sensitive path
// (the folder listing), which would rather go without than wait 30 s.
export async function getActiveDownloads(filterPath = null, { timeoutMs } = {}) {
  const params = filterPath != null ? `?path=${encodeURIComponent(filterPath)}` : '';
  return apiFetch(`/downloads/active${params}`, { timeoutMs });
}

export async function getWaitingDownloads(offset = 0, num = 100, filterPath = null, { timeoutMs } = {}) {
  const params = new URLSearchParams({ offset, num });
  if (filterPath != null) params.set('path', filterPath);
  return apiFetch(`/downloads/waiting?${params}`, { timeoutMs });
}
