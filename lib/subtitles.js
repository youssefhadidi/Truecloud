 ...
 * stream-cache/subs/{md5(fullPath)}/{md5(source, stream, size, mtime)}.vtt.
 *
...
export async function extractSubtitleVtt(fullPath, cacheDir, track, signal) {
  ...
  // Name the output after what it was converted from, not the track id: ids
  // shift when a sidecar is added, and the video or a sidecar can be replaced
  // in place. Either would otherwise keep serving the old conversion.
  const sourcePath = track.source === 'embedded' ? fullPath : track.file;
  const st = await stat(sourcePath);
  const sourceKey = createHash('md5')
    .update(`${sourcePath}|${track.streamIndex ?? ''}|${st.size}|${st.mtimeMs}`)
    .digest('hex');
  const outPath = join(dir, `${sourceKey}.vtt`);
