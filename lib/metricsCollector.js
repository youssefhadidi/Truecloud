...
async function getCpuFreq() {
  // Single-core read: cpu0's scaling_cur_freq is what the kernel cpufreq driver
  // is currently requesting. Representative for the whole package on a NAS where
  // all cores share a governor. Falls back to /proc/cpuinfo's first "cpu MHz" line.
  try {
    const khz = parseInt(
      (await readFile('/sys/devices/system/cpu/cpu0/cpufreq/scaling_cur_freq', 'utf8')).trim(),
      10,
    );
  ...
  try {
    ...
    if (line) return Math.round(parseFloat(line.split(':')[1]) || 0);
  } catch {}

  return 0;
}

// Cached CPU topology: logical CPU index → physical core_id.
// Topology never changes at runtime so we read it once.
let cpuTopology = null;
async function getCpuTopology() {
  if (cpuTopology) return cpuTopology;
  try {
    ...
    const topo = await Promise.all(
      cpus.map(async (c) => {
        try {
          const id = parseInt((await readFile(`/sys/devices/system/cpu/${c}/topology/core_id`, 'utf8')).trim(), 10);
...
async function getCpuTemperature() {
  try {
    ...
    for (const entry of entries) {
      ...
      // Build per-logical-cpu temps: match topology core_id → "Core N" sensor.
      // On AMD chips that only expose Tdie/Tctl, all logical cpus share that value.
      const topology = await getCpuTopology();
      let coreTemps = [];
      if (topology.length > 0) {
        const coreSensorByPhysId = new Map();
        for (const s of sensors) {
          const m = s.label.match(/^Core\s+(\d+)$/i);
          if (m) coreSensorByPhysId.set(parseInt(m[1], 10), s.value);
        }
        if (coreSensorByPhysId.size > 0) {
          coreTemps = topology.map((physId) => (physId != null ? coreSensorByPhysId.get(physId) ?? null : null));
        } else {
          // No per-core sensors (AMD k10temp/zenpower) — fall back to package for every logical cpu
          coreTemps = topology.map(() => pkgSensor.value);
        }
      }

      return { packageTemp: pkgSensor.value, sensors, coreTemps };
    }
...
export async function collectMetrics() {
  if (!isLinux) {
    ...
    return {
      timestamp: Date.now(),
      cpu: {
        cores: cpus.map(() => ({ usage: 0 })),
        overall: 0,
        freq: cpus[0] ? Math.round(cpus[0].speed) : 0,
        loadAvg: os.loadavg(),
      },
  ...
  const [cpuStats, cpuFreq, netStats, diskStats, meminfo, cpuTemp] = await Promise.all([
    parseProcStat(),
    getCpuFreq(),
    parseProcNetDev(),
    parseProcDiskStats(),
  ...
  if (prevCpuStats) {
    ...
    cpuCores = cores.map((core, i) => {
      ...
      return { usage: Math.max(0, Math.min(100, usage)), temp };
    });
    ...
    cpuCores = cpuStats.slice(1).map((_, i) => ({ usage: 0, temp: coreTemps[i] ?? null }));
  }
  ...
  return {
    timestamp: Date.now(),
    cpu: { cores: cpuCores, overall: cpuOverall, freq: cpuFreq, loadAvg: os.loadavg() },
    ram: {
      total: totalRam,
