 ...
 *
 * Recency is seeded from the directory's own timestamp, not left at zero to be
 * raised by whatever files turn up. An HLS job creates its output directory and
 * then waits on the encode semaphore, so a queued stream-copy job (which also
 * clears any stale playlist.m3u8) leaves a genuinely empty directory behind for
 * as long as the job ahead of it runs. Scored from zero that reads as "last
 * touched at the epoch", and the age sweep deletes the directory ffmpeg is
 * about to be spawned into with `cwd` — an ENOENT on a job that had done
 * nothing wrong.
 *
 * The seed is the directory's mtime only, never its atime: reading a
 * directory's entries updates its atime (relatime refreshes it once a day), and
 * this walk reads every one of them each pass. Seeding from atime made every
 * directory look touched within the last day, so the age sweep never fired and
 * the budget sweep evicted in scan order rather than least-recently-watched.
 */
async function measure(path) {
  ...
  let entries;
  try {
    recency = (await stat(path)).mtimeMs || 0;
 ...
 *   {hash}.share.mp4 — share-link faststart copies / MKV remuxes (file)
 *   {hash}.mp4    — legacy full-file remuxes, no longer read; left to age out
 */
