...
// Import shared modules (treated as CommonJS, so use default import)
import thumbnailConfigPkg from '../thumbnailConfig.js';
const { readThumbnailConfig } = thumbnailConfigPkg;

console.error('[WORKER] Thumbnail config imported');

import semaphorePkg from '../semaphore.js';
const { Semaphore } = semaphorePkg;

console.error('[WORKER] Semaphore imported');

// Import shared extension constants
import extensionsPkg from '../extensions.js';
const { IMAGE_EXTENSIONS, VIDEO_EXTENSIONS, PDF_EXTENSIONS, THUMBNAIL_EXTENSIONS, OPTIMIZE_EXTENSIONS, STREAM_EXTENSIONS } = extensionsPkg;
