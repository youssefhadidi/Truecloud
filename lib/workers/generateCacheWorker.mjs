...
const semaphore = new Semaphore(40);
...
async function generateStream(file) {
  ...
  return new Promise((resolve) => {
    ...
    // For MKV files, probe codecs and transcode audio if needed
    if (ext === '.mkv') {
      probeCodecs(file.path).then((codecs) => {
        const needsAudioTranscode = codecs.audioCodec && !BROWSER_AUDIO.has(codecs.audioCodec);
        ...
        ffmpegArgs.push('-movflags', 'faststart', '-y', cachedPath);

        spawnAndWait(ffmpegArgs, resolve);
      });
    } else {
      // For MP4 files, just add faststart
      ffmpegArgs = ['-i', file.path, '-c:v', 'copy', '-c:a', 'copy', '-movflags', 'faststart', '-y', cachedPath];
      spawnAndWait(ffmpegArgs, resolve);
    }
  });
}

// Helper to spawn FFmpeg and wait for completion
function spawnAndWait(ffmpegArgs, resolve) {
  const ffmpeg = spawn('ffmpeg', ffmpegArgs);
  const processId = `ffmpeg-stream-${Date.now()}-${Math.random()}`;
  spawnedProcesses.set(processId, ffmpeg);
...
async function streamNeedsGeneration(file) {
  const cachedPath = join(resolve(cwd, streamCacheDir), `${file.md5}-stream.mp4`);
  try {
    const stats = await fsPromises.stat(cachedPath);
    return !stats.isFile();
  } catch {
