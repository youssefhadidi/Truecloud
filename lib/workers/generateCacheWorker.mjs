...
// Supported extensions (sharp 0.33+ handles HEIC/HEIF natively via libheif)
const IMAGE_EXTENSIONS = ['.jpg', '.jpeg', '.gif', '.bmp', '.png', '.webp', '.ico', '.heic', '.heif'];
const VIDEO_EXTENSIONS = ['.mp4', '.avi', '.mov', '.mkv', '.flv', '.wmv', '.webm', '.m4v', '.mpg', '.mpeg'];
const PDF_EXTENSIONS = ['.pdf'];

const THUMBNAIL_EXTENSIONS = [...IMAGE_EXTENSIONS, ...VIDEO_EXTENSIONS, ...PDF_EXTENSIONS];
const OPTIMIZE_EXTENSIONS = IMAGE_EXTENSIONS;
...
function generatePdfThumbnail(filePath, thumbnailPath, timeoutMs = 60000) {
  ...
  const gsArgs = [
    ...
    '-r72', `-sOutputFile=${jpgPath}`, filePath,
  ];
