...
async function main() {
  try {
    ...
    if (cancelRequested) {
      console.error('[WORKER] Processing was cancelled - sending cancelled status');
      console.error('[WORKER] Counters at cancellation:', counters);
      send({
        status: 'cancelled',
        processed: counters.processed,
        total,
      ...
      console.error('[WORKER] Cancelled status sent, exiting main()');
      return;
    }
