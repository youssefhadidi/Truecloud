...
import { optiCachePath } from '../optiCache.mjs';
import { VIEWER_IMAGE, OPTIMIZE_MIN_BYTES } from '../imageVariants.mjs';
...
function killSpawnedProcesses() {
  for (const [processId, proc] of spawnedProcesses.entries()) {
    try {
      // SIGKILL: a SIGTERM'd HLS ffmpeg writes its playlist trailer, stamping
      // a partial rendition complete. Every other output here is written under
      // a temp name, so nothing half-done is left looking finished.
      proc.kill('SIGKILL');
    } catch (err) {
...
// The file the media viewer's optimize-image request for `file` will look for.
// file.path is already absolute (scanDir is resolved by the generate route).
function viewerImageCachePath(file) {
  return optiCachePath(resolve(cwd, optiCacheDir), file.relativePath, file.path, file, VIEWER_IMAGE);
}
...
async function generateOptimized(file) {
  ...
  // Small files are served as-is by the optimize-image routes
  if (file.size < OPTIMIZE_MIN_BYTES) {
    return { skipped: true, reason: 'too-small' };
  }

  const { quality, width, height } = VIEWER_IMAGE;
  const cachedPath = viewerImageCachePath(file);
...
// The on-demand job's directory for this file's H.264 rendition under the
// current maxHeight (the only variant pre-generated). Shared hash, so the job
// finds what this worker made.
function hlsDirFor(file, maxHeight) {
  return join(resolve(cwd, streamCacheDir), 'hls', hlsRenditionHash(file.path, file, { maxHeight }));
}

// An on-demand job writes a segment every few seconds, so anything written in
// here this recently means one is running (or just queued, having pre-written
// its playlist). Leave it the directory. One queued for longer than this stops
// our encode itself when it reaches the encoder (see WORKER_LOCK).
const ON_DEMAND_ACTIVE_MS = 60_000;

async function onDemandActive(hlsDir) {
  const cutoff = Date.now() - ON_DEMAND_ACTIVE_MS;
  try {
    for (const name of await fsPromises.readdir(hlsDir)) {
      const st = await fsPromises.stat(join(hlsDir, name)).catch(() => null);
      if (st && st.mtimeMs > cutoff) return true;
    }
  } catch {
    // No directory yet
  }
...
async function generateStream(file) {
  ...
  const hlsDir = hlsDirFor(file, maxHeight);
  ...
  if (await onDemandActive(hlsDir)) {
    return { skipped: true, reason: 'playing' };
  }
  ...
  // A full run that did not finish must not leave index.m3u8 behind: ffmpeg
  // writes the playlist trailer (#EXT-X-ENDLIST) on most exits, including
  // errors, and a stamped index.m3u8 is what reads as a finished rendition —
  // the video would stop wherever this run did, for good.
  const discardFullRun = () =>
    partial ? Promise.resolve() : fsPromises.rm(m3u8Path, { force: true }).catch(() => {});
  ...
  // Not after a preemption: the directory belongs to a viewer's job now.
  if (!result.success && !result.preempted && hwaccel === 'vaapi' && !cancelRequested) {
    await discardFullRun();
    result = await new Promise((res) => spawnAndWaitHls(buildArgs('none'), hlsDir, res));
  }

  if (result.preempted) {
    // The on-demand job owns the directory and clears what it doesn't keep.
...
function spawnAndWaitHls(ffmpegArgs, hlsDir, resolve) {
  ...
  // Tells the on-demand job which ffmpeg to stop if a viewer opens this video
  // mid-encode. It removes the lock before killing, so a lock that is gone
  // when ffmpeg exits means we were preempted.
  const lockPath = join(hlsDir, WORKER_LOCK);
  const locked = ffmpeg.pid
    ? fsPromises.writeFile(lockPath, JSON.stringify({ pid: ffmpeg.pid })).then(() => true, () => false)
    : Promise.resolve(false);

  let timedOut = false;
  // SIGKILL, not SIGTERM: a graceful exit writes the playlist trailer.
  const timeout = setTimeout(() => {
    timedOut = true;
    ffmpeg.kill('SIGKILL');
  }, 600000);
  ...
  const finish = async (result) => {
    ...
    clearTimeout(timeout);
    spawnedProcesses.delete(processId);
    const preempted = (await locked) && (await fsPromises.rm(lockPath).then(() => false, () => true));
    if (preempted) resolve({ success: false, preempted: true, error: 'Stopped for on-demand playback' });
    else if (timedOut) resolve({ success: false, error: 'FFmpeg HLS timeout (10 min)' });
    else resolve(result);
  };

  ffmpeg.on('close', (code) =>
    finish(code === 0 ? { success: true } : { success: false, error: `FFmpeg HLS exited with code ${code}` }),
  );
  ffmpeg.on('error', (err) => finish({ success: false, error: err.message }));
}
...
async function optimizedNeedsGeneration(file) {
  if (file.size < OPTIMIZE_MIN_BYTES) return false;
  const cachedPath = viewerImageCachePath(file);
  try {
...
async function streamNeedsGeneration(file) {
  ...
  // Pre-cached under the current settings — on-demand will do the rest.
