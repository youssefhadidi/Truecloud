...
function buildHlsArgs(inputPath, outputM3u8, videoCodec, audioCodec, hwaccel, { maxHeight, sourceHeight, pixFmt, prefetchSecs } = {}) {
  ...
  const hlsTail = [
    ...audioArgs,
    // MPEG-TS muxer defaults (muxpreload 0.5 s, muxdelay 0.7 s) push the PCR
    // ahead of the first video frame, which stalls hls.js on seg000. Force both
    // to 0 so segments start at t=0.
    '-muxdelay', '0',
    '-muxpreload', '0',
    '-f', 'hls',
    '-hls_time', '4',
  ...
  // Skip rescaling when source already fits under the cap — preserves the
  // -c:v copy fast path for H.264 sources at or below the target height.
  ...
  const vaapiEncodeArgs = [
    ...
    '-idr_interval', '1', '-force_key_frames', 'expr:gte(t,n_forced*4)',
  ];

  if (hwaccel === 'vaapi') {
    // Full-HW pipeline for 8-bit 4:2:0 sources the iGPU can decode.
    if (isFullHwCompatible(videoCodec, pixFmt)) {
      ...
      if (needsScale) args.push('-vf', `scale_vaapi=w=-2:h=${maxHeight}`);
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        // Capture size for the name+size thumbnail key (path-independent).
...
async function generateStream(file) {
  ...
  // H.264 + browser-compatible audio in a natively playable container — served directly
  // by the stream route, no HLS cache needed.
  // Note: MKV is Chrome-only; MP4/M4V are universally supported.
  if ((ext === '.mp4' || ext === '.m4v' || ext === '.mkv') &&
