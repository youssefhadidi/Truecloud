...
export default function TranscodingPanel() {
  ...
  const handleSave = () => {
    ...
    saveMutation.mutate(
      { maxHeight: value },
      {
        onSuccess: () => addNotification('success', 'Transcoding settings saved'),
        onError: () => addNotification('error', 'Failed to save transcoding settings'),
      }
    );
  };
    ...
    <div className="max-w-3xl">
      ...
      <div className="bg-gray-800 rounded-lg shadow p-4 sm:p-6">
        <div className="space-y-5">
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-1">Maximum output resolution</label>
            <select
              value={maxHeight}
            ...
            <p className="text-xs text-gray-400 mt-1">Videos smaller than the selected resolution are not upscaled.</p>
          </div>
